...
class TestUpdate_Fact(unittest.TestCase):
    ...
    def test_argspec_no_updates(self):
        ...
        assert "missing required arguments: after" in result["msg"]
    ...
    def test_string(self):
        ...
        assert "-" + before in result["diff_lines"]
        assert "-" + before in result["diff_text"]
        assert "+" + after in result["diff_lines"]
    ...
    def test_diff_list(self):
        ...
        result = self._plugin.run(task_vars=self._task_vars)
        assert result["changed"]
        assert "-3" in result["diff_lines"]
        assert "-3" in result["diff_text"]
        assert "+4" in result["diff_lines"]
        assert "+4" in result["diff_text"]
    ...
    def test_invalid_diff_engine_not_collection(self):
        ...
        result = self._plugin.run(task_vars=self._task_vars)
        assert result["failed"]
        assert "Plugin name should be provided as a full name including collection" in result["msg"]
    ...
    def test_invalid_diff_engine_not_valid(self):
        ...
        result = self._plugin.run(task_vars=self._task_vars)
        assert result["failed"]
        assert "Error loading plugin 'a.b.c'" in result["msg"]
    ...
    def test_invalid_regex(self):
        ...
        result = self._plugin.run(task_vars=self._task_vars)
        assert result["failed"]
        assert "The regex '+', is not valid" in result["msg"]
    ...
    def test_fail_plugin(self):
        ...
        assert "'NoneType' object has no attribute 'diff'" in self._plugin._result["msg"]
